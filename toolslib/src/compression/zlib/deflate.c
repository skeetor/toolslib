...
int ZEXPORT deflateResetKeep (z_streamp strm)
{
    ...
    strm->adler =
#ifdef GZIP
        s->wrap == 2 ? zlib_crc32(0L, Z_NULL, 0) :
#endif
        adler32(0L, Z_NULL, 0);
...
#ifdef GZIP
        if (s->wrap == 2) {
            strm->adler = zlib_crc32(0L, Z_NULL, 0);
            put_byte(s, 31);
            put_byte(s, 139);
            put_byte(s, 8);
            if (s->gzhead == Z_NULL) {
                ...
                if (s->gzhead->hcrc)
                    strm->adler = zlib_crc32(strm->adler, s->pending_buf,
                                        s->pending);
                s->gzindex = 0;
            ...
            while (s->gzindex < (s->gzhead->extra_len & 0xffff)) {
                if (s->pending == s->pending_buf_size) {
                    if (s->gzhead->hcrc && s->pending > beg)
                        strm->adler = zlib_crc32(strm->adler, s->pending_buf + beg,
                                            s->pending - beg);
                    flush_pending(strm);
                    beg = s->pending;
                    if (s->pending == s->pending_buf_size)
    ...
    unsigned size)
{
    ...
    if (strm->state->wrap == 1) {
        ...
        strm->adler = zlib_crc32(strm->adler, buf, len);
    }
#endif
    strm->next_in  += len;
    strm->total_in += len;
